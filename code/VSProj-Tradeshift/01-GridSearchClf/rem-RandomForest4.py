...
clfFolder = "..\\..\\..\\classifier\\RandomForest1\\"
...
def add_dummies(indata):
    cols_for_dummies = ['x4','x35', 'x65','x95']
    dummies = [pd.get_dummies(indata[col], prefix = col) for col in cols_for_dummies]
    outdata = indata.join(dummies)
    print outdata.shape
    del dummies[:]
    del indata
    gc.collect()

    outdata.drop(cols_for_dummies, axis=1, inplace = True)
    return outdata
...
def clean_features(df):
    ...
    colsDrop = ['x3', 'x34', 'x61', 'x64', 'x91', 'x94']
