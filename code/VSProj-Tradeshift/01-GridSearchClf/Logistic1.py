...
clfFolder = "..\\..\\classifier\\LogisticRegression\\"

def cv_optimize(X_train, Y_train, clf):
    alpha_range = 10.0 ** np.arange(-4, -1)
    #alpha_range = [0]
    param_grid = dict(alpha=alpha_range)

    gs = GridSearchCV(clf, param_grid = param_grid, cv = 10, n_jobs = 2, verbose = 3)
    gs.fit(X_train, Y_train)
    print "gs.best_params_ = {0}, gs.best_score_ = {1}".format(gs.best_params_, gs.best_score_)
    return gs.best_estimator_, gs.best_params_, gs.best_score_

def fit_clf(X_train, Y_train):
    clf = SGDClassifier(loss = 'log', n_iter = 10, shuffle = True, random_state = 96, alpha = 0.1) 
